import React from 'react';
import {
    View,
    StyleSheet,
    Text,
    TouchableOpacity,
    Modal
} from 'react-native';

const ButtonLevelSelect = (props) => {
    const stylesPerso = [styles.button];
    let diffLevel = 0.1;
    let labelText = 'Fácil';
    if (props.bgEasy) {
        stylesPerso.push(styles.bgEasy);
    }
    if (props.bgNormal) {
        stylesPerso.push(styles.bgNormal);
        diffLevel = 0.2;
        labelText = 'Intermediário';
    }
    if (props.bgHard) {
        stylesPerso.push(styles.bgHard);
        diffLevel = 0.3;
        labelText = 'Difícil'
    }
    return (
        <TouchableOpacity style={stylesPerso} onPress={() => props.onLevelSelected(diffLevel)}>
            <Text style={styles.buttonLabel}>{labelText}</Text>
        </TouchableOpacity>
    )
}

export default props => {
    return (
        <Modal onRequestClose={props.onCancel} visible={props.isVisible} animationType='slide' transparent={true}>
            <View style={styles.frame}>
                <View style={styles.container}>
                    <Text style={styles.title}>Selecione o Nível</Text>
                    <ButtonLevelSelect bgEasy onLevelSelected={props.onLevelSelected}/>
                    <ButtonLevelSelect bgNormal onLevelSelected={props.onLevelSelected}/>
                    <ButtonLevelSelect bgHard onLevelSelected={props.onLevelSelected}/>
                </View>
            </View>
        </Modal >
    )
}

const styles = StyleSheet.create({
    container: {
        backgroundColor: '#eee',
        alignItems: 'center',
        justifyContent: 'center',
        padding: 15,
    },
    title: {
        fontSize: 30,
        fontWeight: 'bold'

    },
    button: {
        marginTop: 10,
        padding: 5
    },
    buttonLabel: {
        fontSize: 20,
        color: '#eee',
        fontWeight: 'bold',
    },
    bgEasy: {
        backgroundColor: '#49b65d'
    },
    bgNormal: {
        backgroundColor: '#2765f7'
    },
    bgHard: {
        backgroundColor: '#f26337'
    },
    frame: {
        flex: 1,
        alignItems: 'center',
        justifyContent: 'center',
        backgroundColor: 'rgba(0,0,0,0.6)',
    }
})


/*
                    <TouchableOpacity style={[styles.button, styles.bgEasy]} onPress={() => props.onLevelSelected(0.1)} >
                        <Text style={styles.buttonLabel}>Fácil</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.button, styles.bgNormal]} onPress={() => props.onLevelSelected(0.2)} >
                        <Text style={styles.buttonLabel}>Intermediário</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.button, styles.bgHard]} onPress={() => props.onLevelSelected(0.3)} >
                        <Text style={styles.buttonLabel}>Difícil</Text>
                    </TouchableOpacity>
*/